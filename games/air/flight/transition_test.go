...
// The seams between the powered-approach and up-and-away laws (#203/#204). The
// stick is held displaced through each crossing because the trim integral only
// feeds the command path while the stick flies the jet.
...
func TestLawHysteresis(t *testing.T) {
	...
	for i := 0; i < 240*30; i++ { // half a minute hovering about the old boundary
		...
		// Proportional speed hold centred on the old 130 boundary - the chatter case.
		// A fixed bang-bang throttle cannot bracket the level drag point.
...
// throttle cannot capture on-speed. TestApproachPowerHold flies it with power.

// TestApproachPowerHold validates the client's ATC law (#202) against the real
// core: the constants must mirror apps/air/web/src/game/atc.ts. Catches the
// two-controller failure where the PA and ATC laws both steer alpha.
...
// TestBallDatum: the approach alpha datum must not move with speed. Within the
// approach band the law is AoA-referenced - the datum stays at on-speed and
// power flies the path; the residual ripple is the alpha loop on the phugoid.
