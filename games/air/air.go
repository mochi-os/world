...
const (
	...
	threat_confirm = 0.25   // s a missile threat must PERSIST before the defensive programme runs, EVERY TIER ALIKE (#216). react is the human's perception-to-action latency, but it was doing a second job nobody had named: b.alert resets the moment the threat test flickers off, so react was also the only DEBOUNCE on that test. Setting it to zero for the machine removed the debounce along with the latency, and the machine committed the whole aircraft - burner cut, press cancelled, the fight decision preempted - to rounds that were in the window for a tick. Measured superhuman v ace over 16 heater seeds: evading 7.82% of the fight against the ace's 0.13%, arriving 41 kt slower, surviving 2 of 16 against 10. Every human tier already exceeds this floor (ace 0.4, pilot 1.0, novice 2.0), so it moves the machine alone while leaving its reflex intact - a SHARED floor rather than a machine branch, because a machine-only doctrine number is what cost #106 its BVR depth 18-29. Swept rather than picked: on the wide missile ladder 0/0.20/0.25/0.35 read 20-28 (failing), 26-21, 28-19, 26-22, and on the duel ladder 6-10 at 18.8 s, 7-9 at 17.7, 9-6 at 34.1, 8-8 at 17.0 - 0.25 is the optimum on both
